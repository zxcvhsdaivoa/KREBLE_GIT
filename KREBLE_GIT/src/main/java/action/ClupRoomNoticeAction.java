...
public class ClupRoomNoticeAction  implements Action {
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(login_id==null) {
			...
			if(ismember==1) {
				...
				ClupNoticeService cns = new ClupNoticeService();
				request.setAttribute("clup_notice",cns.selectNotice(clup_no));
				forward.setPath("/clup_room_notice.jsp");
			}
			else {
